...
class PacmanGraphics:
    ...
    def update(self, state):
        ...
        if self.agent_counter == 0:
            self.turn += 1
            if DISPLAY_MOVES:
                ghosts = [pacman.nearest_point(state.getGhostPosition(i)) for i in range(1, num_agents)]
                print("%4d) P: %-8s" % (self.turn, str(pacman.nearest_point(state.getPacmanPosition()))),
